...
$ExitWorkingSet = 137
$ExitOutputLimit = 138
$WallLimitSeconds = 60.0
$QualificationMarginSeconds = 20.0
$WorkingSetLimitBytes = [int64]2 * 1024 * 1024 * 1024
$OutputLimitBytes = [int64]64 * 1024 * 1024
$TempPrefix = "lean-rgc-uprime-wp4h-recovery-"
$FrozenTest = "tests/test_odlrq_hankel_predictive.py"

function Write-RunnerError {
    param([Parameter(Mandatory = $true)][string] $Message)
    [Console]::Error.WriteLine("uprime-wp4h-recovery: {0}", $Message)
}
...
function Write-CapturedStream {
    ...
    if ($item.Length -ge $OutputLimitBytes) {
        [Console]::Error.WriteLine(
            "uprime-wp4h-recovery: captured {0} omitted after output cap",
            $Stream
        )
        return
    }
...
try {
    ...
    $expectedScript = [IO.Path]::GetFullPath((Join-Path $repoRoot "tools\run_uprime_wp4_h_recovery_tests.ps1"))
    ...
    if (-not (Test-Path -LiteralPath $testCandidate -PathType Leaf)) {
        Write-RunnerError "the one frozen WP4-H test module is missing"
...
def _deny_subprocess(*_args, **_kwargs):
    raise RuntimeError("WP4-H semantic tests may not spawn subprocesses")
...
_receipt = os.environ.pop("UPRIME_WP4H_EXIT_RECEIPT", None)
if not _receipt:
...
'@
    ...
    $env:UPRIME_WP4H_SUBPROCESS_POLICY = "forbid"
    $env:UPRIME_WP4H_EXIT_RECEIPT = $exitReceiptPath
    ...
    if ($capFailure -eq "timeout") {
        Write-RunnerError "60-second hard wall exceeded; Python was terminated"
    ...
    elseif ($stopwatch.Elapsed.TotalSeconds -gt $QualificationMarginSeconds) {
        Write-RunnerError (
            "20-second qualification margin exceeded ({0:N3} seconds); hard wall unchanged" -f `
        ...
        $qualificationMessage = (
            "uprime-wp4h-recovery: qualification elapsed={0:N3}s peak_working_set={1}" -f `
    ...
    Remove-Item Env:UPRIME_WP4H_EXIT_RECEIPT -ErrorAction SilentlyContinue
