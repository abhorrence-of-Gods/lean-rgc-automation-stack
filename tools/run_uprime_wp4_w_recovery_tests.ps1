...
$TempPrefix = "lean-rgc-uprime-wp4w-recovery-"
$FrozenTest = "tests/test_odlrq_componentwise_window.py"

function Write-RunnerError {
    param([Parameter(Mandatory = $true)][string] $Message)
    [Console]::Error.WriteLine("uprime-wp4w-recovery: {0}", $Message)
}
...
function Write-CapturedStream {
    ...
    if ($item.Length -ge $OutputLimitBytes) {
        [Console]::Error.WriteLine(
            "uprime-wp4w-recovery: captured {0} omitted after output cap",
            $Stream
        )
        return
    }
...
try {
    ...
    $expectedScript = [IO.Path]::GetFullPath((Join-Path $repoRoot "tools\run_uprime_wp4_w_recovery_tests.ps1"))
    ...
    if (-not (Test-Path -LiteralPath $testCandidate -PathType Leaf)) {
        Write-RunnerError "the one frozen WP4-W test module is missing"
...
def _deny_subprocess(*_args, **_kwargs):
    raise RuntimeError("WP4-W semantic tests may not spawn subprocesses")
...
_receipt = os.environ.pop("UPRIME_WP4W_EXIT_RECEIPT", None)
if not _receipt:
...
'@
    ...
    $env:UPRIME_WP4W_SUBPROCESS_POLICY = "forbid"
    $env:UPRIME_WP4W_EXIT_RECEIPT = $exitReceiptPath
    ...
    elseif ($stopwatch.Elapsed.TotalSeconds -gt $QualificationMarginSeconds) {
        ...
        $qualificationMessage = (
            "uprime-wp4w-recovery: qualification elapsed={0:N3}s peak_working_set={1}" -f `
    ...
    Remove-Item Env:UPRIME_WP4W_EXIT_RECEIPT -ErrorAction SilentlyContinue
