...
partition, feature, query, or report.  If an exact distinguishable seeded pair
has an available candidate that the learned 16-step schedule does not select,
the miss counts against learner performance rather than becoming a
prerequisite block.  L0's verified region projection is defined relative to the
same complete query catalogue, so this phase cannot diagnose query-language
inadequacy without circularity.  That question is deferred to an L1 authority
with an independent Lean-oracle witness language.
...
after the node/edge cap checks.  A region above the cap, an unseen required
kind, an unseen runtime instance exceeding a snapshot cap after a valid global
cap-profile preflight, a stage resource cap, or a declared censor produces
`ABSTAIN` and remains in the fixed denominator.  If the frozen fixture's
mechanically required state/row bound already exceeds its own declared global
cap, no denominator can be constructed and preflight returns
`PREREQUISITE_BLOCKED` instead.  Normal completion at budget step 16 and the
short-universe plateau in section 9 are not abstentions.  Attempting a 17th
candidate observation or repeating a candidate is `CONTRACT_FAIL`.  A valid
matrix, admission, snapshot-size, or resource prerequisite that cannot be met
is `PREREQUISITE_BLOCKED`; an existing but unselected distinguisher is a
learner miss.  The implementation must not truncate,
...
are never pooled across instances or families.  An abstained or
censored instance retains its last defined partition/loss and remains in the
16-instance denominator.  Coverage numerator, abstention count, and censor
count are reported over that same denominator.
...
locality abstention count, and strict loss improvement at at least one `t`.
All 16 held-out instances and all eight family aggregates remain in every
denominator.  No selective-family, pooled-member, or complete-case denominator
is allowed.
...
tests.  `NO_CLEAR_GAIN` is a valid complete run with no deterministic strict
gain and no degradation; it carries diagnostic
`L0_STRUCTURAL_HEURISTIC_NOT_SUPPORTED`.  `DEGRADED` is reachable whenever a
frozen checkpoint is worse, coverage falls, or abstention rises.
`PREREQUISITE_BLOCKED` covers cap, matrix, admission, or resource prerequisites;
...
A2 may correct only document/matrix identity defects before B source.  B2 may
correct only the six Phase-B control paths.  C2 may correct only the six semantic
paths without changing the matrix, public meaning, evidence tier, caps,
denominators, query universe, or result rule.  D2 may correct only the two
Phase-D paths and must deterministically regenerate the same result semantics
from the same accepted C and matrix; it cannot change a disposition to obtain
a green result.  A failed ref stays immutable.
...
No theoretical stop condition was found.  The approved next action is A
freeze, exact A-CI classification, then B control bootstrap.  Source C must not
begin before B is green.
